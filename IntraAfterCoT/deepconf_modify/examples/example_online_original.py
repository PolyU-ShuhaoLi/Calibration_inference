...
/eds-storage/shuhaoli_calibration/qwen2.5-1.5B-math
/eds-storage/shuhaoli_calibration/LLaMA-Factory/saves/Qwen2.5-Math-1.5B/full/train_qwq_sft
/eds-storage/shuhaoli_calibration/CANON/checkpoints/grpo_from_sft_qwq

python examples/example_online.py   --qid_start 0 --qid_end 10 --rid base   --dataset /code/deepconf/examples/test_converted.jsonl   --model /eds-storage/shuhaoli_calibration/qwen2.5-1.5B-math   --model_type gpt   --total_budget 128   --output_dir online-base
...
python examples/example_online.py \
  ...
  --model /eds-storage/shuhaoli_calibration/qwen2.5-1.5B-math \
  --total_budget 128 \
  --output_dir online-base


python examples/example_online.py \
  --qid 0 --rid sft \
  --dataset /code/deepconf/examples/test_converted.jsonl \
  --model /eds-storage/shuhaoli_calibration/LLaMA-Factory/saves/Qwen2.5-Math-1.5B/full/train_qwq_sft \
  --total_budget 256 \
  --output_dir online-sft


python examples/example_online.py \
  --qid 0 --rid rl \
  --dataset /code/deepconf/examples/test_converted.jsonl \
  --model /eds-storage/shuhaoli_calibration/CANON/checkpoints/grpo_from_sft_qwq \
  --total_budget 256 \
  --output_dir online-rl
...
def print_evaluation_report(question, ground_truth, evaluation, confidence_eval, result):
    ...
    if correct_evals:
        ...
        best_conf = best_method[1].get("confidence")
        conf_str = f"{best_conf:.3f}" if best_conf is not None else "-"
        print(f"Best correct method: {best_method[0]} (confidence: {conf_str})")
...
def main():
    ...
    parser.add_argument('--window_size', type=int, default=512,
                       help='Sliding window size for confidence computation')
    parser.add_argument('--max_tokens', type=int, default=32768,  # 16000
                       help='Maximum tokens per generation')
    parser.add_argument('--model_type', type=str, default="deepseek", choices=["deepseek", "gpt", "qwen"],
    ...
    def annotate_traces(traces, ground_truth, base_id=0):
        for i, trace in enumerate(traces):
            pred = trace.get("extracted_answer")
            if pred is None:
                is_correct = False
            else:
                try:
                    is_correct = equal_func(pred, ground_truth)
                except Exception:
                    is_correct = (str(pred) == str(ground_truth))
            trace["is_correct"] = is_correct
            trace["trace_id"] = base_id + i

    if ground_truth:
        # 给三份 trace 都加标注（online 会同时有 warmup/final/all）
        annotate_traces(result.warmup_traces, ground_truth, base_id=0)
        annotate_traces(result.final_traces, ground_truth, base_id=len(result.warmup_traces))
        annotate_traces(result.all_traces, ground_truth, base_id=0)

        # 可选：如果你也想把 all_voting_traces 存进 pkl（to_dict 默认不会存）
        if hasattr(result, "all_voting_traces"):
            annotate_traces(result.all_voting_traces, ground_truth, base_id=0)
        '''
        if hasattr(result, "all_voting_traces"):
            result_data["all_voting_traces"] = result.all_voting_traces
        '''
