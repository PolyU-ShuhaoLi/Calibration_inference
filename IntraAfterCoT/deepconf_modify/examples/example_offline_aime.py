...
'''
  ...
  python examples/example_offline_original.py \
    --qid 1 \
    --rid test1 \
    --dataset /code/deepconf/examples/aime_2025.jsonl \
    --model /code/Qwen3-14B \
    --model_type qwen \
    --tensor_parallel_size 1 \
    --budget 1 \
    --output_dir offline-base-14B
...
python examples/example_offline_original.py \
  ...
  --model_type qwen \
  --budget 256 \
  --output_dir offline-base-10 > math.txt
...
python examples/example_offline_original.py \
  ...
  --model_type qwen \
  --budget 256 \
  --output_dir offline-base-10 > base.txt
...
python examples/example_offline_original.py \
  ...
  --model_type qwen \
  --budget 256 \
  --output_dir offline-rl-10 > rl.txt
...
python examples/example_offline_original.py \
  ...
  --rid range0_10 \
  --dataset /code/deepconf/examples/test_converted.jsonl \
  --model /eds-storage/shuhaoli_calibration/LLaMA-Factory/saves/Qwen2.5-1.5B/full/train_7b_expert \
  --model_type qwen \
  --budget 256 \
  --output_dir offline-sft-10 > sft.txt
...
def main():
    ...
    parser.add_argument('--model_type', type=str, default="qwen", choices=["deepseek", "gpt", "qwen"],
    ...
    parser.add_argument('--window_size', type=int, default=1024,
                       help='Sliding window size for confidence computation')
    parser.add_argument('--max_tokens', type=int, default=8192, # 16000    32768    8192
                       help='Maximum tokens per generation')
    ...
    parser.add_argument('--top_p', type=float, default=0.95,
                       help='Top-p sampling parameter')
    parser.add_argument('--repetition_penalty', type=int, default=1.0,
                       help='repetition penalty')
    '''
    其他模型用的参数
